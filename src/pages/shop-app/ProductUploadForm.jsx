...
const categoryOptions = {
    "/fundi-portal/products": ["Steel Window", "Wooden Windows", "Wooden Doors", "Gates", "Gypsum Ceiling", "Steel Doors", "Bamboo Gates"],
    "/professional/professional-products": ["Mansionettes", "Bungalows", "Apartments", "Commercials", "Socials"],
    "/contractorPortal/products": 
    [
...
const ProductUploadForm = () => {
    ...
    const [fromPath] = useState(location.state?.from || "/fundi-portal/products"); // Default to Fundi if not provided
    ...
    // value of role is Professional
    const options = categoryOptions[fromPath] || [];
        ...
        if (isEditing) {
            const updatedProducts = allProducts.map(product =>
                product.id === formData.id
                    ? { ...product, ...formData, images, status: statusType, productDesc, fromPath, options }
                    : product
            );
        ...
        */}
        <div className="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6 space-y-6 mt-2">
            ...
            <form className="grid grid-cols-1 gap-4 mb-8">
                <div>
                    <h2 className="block font-semibold mb-2">Category</h2>
                    <select 
                        name="category"
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        { name: 'price', label: 'Price (KES)', required: true },
                            ...
                            // Skip price input for hardware portal
                            if (name === 'price' && fromPath === "/hardwarePortal/products") {
                                return null; 
                            }
                            return (
                ...
                <div className="flex gap-4 mt-4">
                    <button
                        ...
                        type="button" 
                        className="px-4 py-2 rounded-lg text-white bg-gray-600 hover:bg-gray-500 cursor-pointer hover:border-0"
                    >
                        Preview
                    </button>
                    <button
                        ...
                        className="rounded-lg text-white px-4 py-2 bg-[rgb(0,0,112)] hover:bg-blue-300 hover:text-black cursor-pointer"
                    >
                    Save Changes
                    </button>
                    <button
