...
const HardwareCart = () => {
  ...
  const handleCheckout = () => {
    ...
    navigate("/customer/hardware/checkout");
  };
  ...
  return (
    <section className="container mx-auto mt-10">
      ...
      <nav className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 px-4 mt-28">
        ...
        <Link to="/hardware_shop/hardware-products">
          <div className={`flex flex-row md:flex-col justify-center items-center rounded-lg shadow-md p-4
            ${isActive.startsWith("/customer/hardware") ? "bg-[rgb(0,0,122)] text-white" : "bg-blue-200 text-gray-900"}`}>
      ...
      <div className="flex justify-between px-4 my-5">
        ...
        <button 
          type='button' 
          onClick={() => navigate("/hardware_shop")}
      ...
      <section className="flex flex-col sm:flex-row justify-between items-start gap-6">
        ...
        <div className="w-[90%] mx-auto sm:w-1/3 bg-white p-4 rounded-md shadow-md self-start my-4">
          <h1 className="text-lg font-semibold mb-3">CART SUMMARY</h1>
          {/* <div className="flex justify-between text-gray-700 text-lg border-b pb-2">
            <h3 className="font-semibold">Subtotal</h3>
            <p className="font-semibold">Total: Ksh {totalPrice.toLocaleString()}</p>
          </div>

          <br />
          <p className="text-xs text-gray-500 mt-1">(Inclusive of VAT)</p>

          <p className="font-semibold text-gray-700">Subtotal: Ksh {totalPrice.toLocaleString()}</p>
          {/* <p className="font-semibold text-gray-700">Tax (16%): Ksh {tax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p> */}
          {/* <p className="font-semibold text-gray-700">Shipping: Ksh {shipping.toLocaleString()}</p>
          <hr className="my-4" /> */}
          ...
          <button 
            type="button" 
            className="mt-4 w-full bg-[rgb(0,0,122)] text-white py-2 rounded-md text-sm font-medium hover:opacity-90 cursor-pointer"
