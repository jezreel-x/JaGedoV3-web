...
const statusMap = {
  New: ["Under Review", "Unreviewed"],
  Drafts: ["Draft"],
  Past: ["Past"],
  Bids: [ " Under Evaluation", "Bid Invited", "Bid Submitted", "Invited"],
  Active: ["Ongoing", "Started", "Pending Start"],
};

// Generate mock data for each tab with orders labeled 1–10 per tab
const generateData = () => {
  const result = [];
  ...
  Object.entries(statusMap).forEach(([tabName, statuses]) => {
    for (let i = 0; i < 4; i++) {
      ...
      result.push({
        ...
        category: ["Machinery",  "Hardware","Custom Product" ,"Design"][i % 4],
        type: "Order",
        managedBy: [ "JaGedo"][i % 1],
...
export default function AdminPortal() {
  ...
  const [filters, setFilters] = useState({ category: "", type: "", managedBy: "" });
  ...
  const filteredData = tableData.filter(
    (row) =>
      ...
      (!filters.date || new Date(row.date).toISOString().split("T")[0] === filters.date)
  );
          ...
          {navItems.map((nav) => (
            <button
              ...
              className={`px-6 py-2 rounded-md font-bold w-full text-center transition-colors duration-200 ${
                activeTab === nav.name ? "bg-[rgb(0,0,122)] text-white" : "bg-blue-200 text-black"
              }`}>
              {nav.name} ({nav.count})
            </button>
          ))}
  ...
  {paginatedData.length > 0 ? (
    paginatedData.map((row, rowIndex) => {
        ...
        if (activeTab === "New") {
          switch (row.req) {
            case "Order 1":
              ...
              break;
            case "Order 2":
              navigate("/admin-assign-hardware-product-competitive");
              break;
            case "Order 3":
          ...
          switch (row.req) {
            ...
            case "Order 2":
              navigate("/admin-assigned-Hardware1-order");
              break;
            case "Order 3":
      ...
      const isClickableReq =
        (activeTab === "New" &&
          ["Order 1", "Order 2", "Order 3", "Order 4"].includes(
            row.req
          )) ||
        (activeTab === "Bids" &&
          ["Order 1", "Order 2", "Order 3", "Order 4"].includes(
